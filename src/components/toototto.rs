...
fn make_computer_move(
    ...
    if matches!(*game_state, GameState::Ongoing) {
        ...
        match game_difficulty {
            ...
            Difficulty::Hard => {
                // let otto = vec![Cell::O, Cell::T, Cell::T, Cell::O];
                // let toot = vec![Cell::T, Cell::O, Cell::O, Cell::T];

                // let check_win = |board: &Vec<Vec<Cell>>, word: Vec<Cell>, cell: Cell, block: bool| -> Option<Vec<Vec<Cell>>> {
                //     let rows = board.len();
                //     let cols = board[0].len();
                ...
                // // 1. Check if there is a way for it to win by spelling OTTO by placing an O
                // if let Some(new_board) = check_win(&board, otto.clone(), Cell::O, false) {
                //     return Some(new_board);
                // }

                // // 2. Check if there is a way for it to win by spelling OTTO by placing a T
                // if let Some(new_board) = check_win(&board, otto, Cell::T, false) {
                //     return Some(new_board);
                // }

                // // 3. Check if there is a way for it to block the player from spelling TOOT by placing a T
                // if let Some(new_board) = check_win(&board, toot.clone(), Cell::T, true) {
                //     return Some(new_board);
                // }

                // // 4. Check if there is a way for it to block the player from spelling TOOT by placing an O
                // if let Some(new_board) = check_win(&board, toot, Cell::O, true) {
                //     return Some(new_board);
                // }

                // // 5. If none of those 4 options exist, will run the for loop that currently exists, it will just place random letter
                // let computer_cell = if rng.gen_bool(0.5) { Cell::T } else { Cell::O };
